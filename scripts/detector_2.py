...
class ProductDetector2:
    ...
    def run(self):
        if self.classifier.get_class_to_find() is not None:
            ...
            pil_rgb_image = PIL.Image.fromarray(rgb_image[..., ::-1])  # Convert to PIL image

            # Before calling rotate_image, convert the PIL Image back to an OpenCV-compatible format
            opencv_rgb_image = np.array(pil_rgb_image)

            if self.rotate:
                rotated_image, angle = self.rotation_compensation.rotate_image(opencv_rgb_image, time_stamp)
                ...
                if self.visualize_results:
                    ...
                    self.visualization_pub.publish(self.bridge.cv2_to_imgmsg(result, "bgr8"))
            else:
