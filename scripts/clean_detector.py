...
class ProductDetector:
    def __init__(self, yolo_weights_path, pmf_weights_path) -> None:
        ...
        self.result_publisher = rospy.Publisher("/product_detector/result_image", CompressedImage)
        self.curent_class_publisher = rospy.Publisher("/product_detector/current_class", String)
        self.yolo = YoloHelper(yolo_weights_path, bounding_box_conf_threshold=0.6, device="cuda:0", image_loader=IMAGE_LOADER)
        self.classifier = PMF(pmf_weights_path, classification_confidence_threshold=0.8,
                              image_loader=IMAGE_LOADER, device="cuda:0")

    def rgb_callback(self, msg):
        self.rgb_image = self.bridge.imgmsg_to_cv2(msg, desired_encoding="bgr8")
        rotated_image, self.angle = self.rotation_compensation.rotate_image(self.rgb_image, msg.header.stamp)
        self.rotated_image = PIL.Image.fromarray(rotated_image[..., ::-1])  # Convert to PIL image
    ...
    def run(self):
        if self.classifier.get_current_class() is None:
            ...
            cropped_images, bounding_boxes = self.yolo.predict(source=self.rotated_image, agnostic_nms=True)

            # Step 2: Binary classification according to currently requested class
            scores, labels = self.classifier(cropped_images)
            ...
            self.curent_class_publisher.publish(String(data=self.classifier.current_class))
